...
network_analysis_tool/
  ...
  .flask_secret_key (created at runtime unless env secret provided)
  check_setup.py
  dns_resolver.py
  generate_report.py
  install.py
...
### Helper scripts
- `check_setup.py`: checks Python version (`3.10+`), validates imports for all packages declared in `requirements.txt`, and reports admin status.
...
- `APP_SECRET_KEY` / `FLASK_SECRET_KEY` / `SECRET_KEY` can override Flask secret key
- `ACTION_TOKEN` protects destructive endpoints (`/api/kill`, `/api/block`)

Secret key behavior:
- If secret env var is present, app uses it.
- Otherwise app loads key from `.flask_secret_key`.
- If key file does not exist, app generates a strong key and saves it on first run.
...
- Resolve reverse DNS via `DNSResolver.resolve_ip(remote_ip)` for `remote_domain` (with IP fallback).
...
Note:
- `app.py` export route now writes both CSV and JSON summary output.
...
Auth behavior:
- If `ACTION_TOKEN` is configured, request must include header `X-Action-Token: <token>`.
- If `ACTION_TOKEN` is not configured, endpoint is allowed only for strict localhost requests.
...
Important:
- Route writes both files:
  - CSV connection export (`connections_<timestamp>.csv`)
  - JSON summary export (`network_summary_<timestamp>.json`)
...
Implemented table and modal features in JS:
- `sortTable(...)` performs real row sorting for both connections and processes.
- `filterConnections()` applies text/risk/process filters and re-renders the table.
- `filterProcesses()` applies live text filtering and re-renders the table.
- `showWhoisInfo(ip)` calls `/api/whois`, opens modal, renders results, and handles errors.
...
- Flask secret key is not hardcoded and is generated/persisted when not provided via env,
- destructive endpoints use action-token authentication for non-local exposure.

Security gaps:
- no role-based authorization model,
- no rate limiting for API endpoints,
- no TLS termination built into the app server (should be handled by reverse proxy/platform).

## 12. Known Inconsistencies and Limitations

1. `main.py` is removed and retained only as a placeholder marker file.
2. `test_startup.py` references removed GUI symbols and missing imports.
3. `monitor.py` stores connection byte fields but does not update per-connection sent/recv values from socket-level counters.
4. Process byte stats are approximated by distributing system totals across process connection counts.
5. `protocol` assignment in monitor checks `conn.type.upper()`; for numeric socket type values this falls back to `TCP`.
6. `app.py` path injection logic (`TOOL_DIR = Path(__file__).parent.parent / "network_analysis_tool"`) is redundant for this repository layout.
7. Desktop settings are persisted as plain JSON (`desktop_settings.json`) without schema versioning or migration logic.
...
1. Add authentication and confirmation UX in the frontend before kill/block calls.
2. Add API rate limiting for destructive and expensive endpoints.
3. Improve per-connection byte accounting beyond process-level approximation.
4. Add automated tests for API contracts and monitor behavior.
5. Remove or fully rewrite legacy `test_startup.py`.
6. Simplify or remove redundant path-injection logic in `app.py`.
